...
export class Game extends Phaser.Scene {
  ...
  create() {
    ...
    this.setupInput();
    this.computeFlow();
    const p = iso(this.p.x, this.p.y);
    this.camX = p.x - 64;
    this.camY = p.y - 70;
    this.camV = { x: 0, y: 0 };
    this.camS = { x: Math.round(this.camX), y: Math.round(this.camY) };
    this.glyphs = {};
    for (const c of "mcw")
      this.glyphs[c] = this.make.bitmapText(
        { font: "font_" + c, text: "x", size: 8 },
        false,
      );
    this.swordStamp = this.make
      .image({ key: "sword" }, false)
      .setOrigin(0.15, 0.5);
    if (!this.scene.isActive("ui")) this.scene.launch("ui");
    this.scene.bringToTop("ui");
    this.say(this.lvl === 1 ? "descend!" : "floor B" + this.lvl, 2.5);
  }
  ...
  update(time, delta) {
    ...
    if (!this.p.dead && (JD(this.keys.I) || JD(this.keys.TAB) || this.touchI))
    ...
    if (this.descending) {
      ...
      if (this.wipe >= 1 && !this.restarting) {
        ...
        this.scene.restart({ depth: this.lvl + 1, player: this.stats() });
        return;
      }
    }
    ...
    for (const o of this.props) {
      ...
      if (o.flash > 0) o.spr.setTintFill(COL.w);
